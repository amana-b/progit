Public Class frmBalanceClosing
    Inherits BaseUI.BaseForm

#Region "ﾌﾚｰﾑﾜｰｸ"
    Private Frame As NodeCore.Frame.Master.BalanceClosing
#End Region
...
#Region "定型準備"
    ...
    Protected Overrides Sub ControlInit()
        ...
        ' ﾃｷｽﾄﾎﾞｯｸｽ
        Me.txt得意先CD.PropetyInitForm(BaseContents.Field.LockType.Key, True, Windows.Forms.ImeMode.Disable, BaseContents.TextBox.TextStyle.Code, NodeUI.Help.Type.M得意先, Me.txt得意先CD.Name, "")
        Me.txt締切日付.PropetyInitForm(BaseContents.Field.LockType.Data, True, Windows.Forms.ImeMode.Disable, BaseContents.TextBox.TextStyle.YYYYMMDD, NodeUI.Help.Type.None, "", "")
        Me.txt締切残高.PropetyInitForm(BaseContents.Field.LockType.Data, False, Windows.Forms.ImeMode.Disable, BaseContents.TextBox.TextStyle.Currency, NodeUI.Help.Type.None, "", "")
    ...
    Protected Overrides Sub FrameInit()
        ...
        ' ｺﾝｽﾄﾗｸﾀ
        '-------------------------------------
        Frame = New NodeCore.Frame.Master.BalanceClosing(Me.Security, Me.Config)
        ...
        Frame.Fields.Header.締切日付.Control = Me.txt締切日付
        Frame.Fields.Header.締切残高.Control = Me.txt締切残高
    End Sub
#End Region
...
#Region "ﾍﾟｰｼﾞｲﾍﾞﾝﾄ"
    ...
    Private Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        ' 明細行初期化
        '----------------------------------------------------------------------
        Me.txt得意先CD_AfterUpdate(Me, System.EventArgs.Empty)
...
#Region "ｸﾞﾘｯﾄﾞ関連"
    ...
    Private Sub grdDetall_Setting(ByRef TextBoxAddTop As BaseUI.Control.TextBox, ByRef TextBoxAddBtm As BaseUI.Control.TextBox, ByRef ButtonInsert As BaseUI.Control.Button, ByRef ButtonDelete As BaseUI.Control.Button) Handles grdDetall.Setting
        ...
        ' 共通処理
        '----------------------------------------------------------------------
        TextBoxAddTop = Me.txt締切日付
        TextBoxAddBtm = Me.txt締切残高
        ...
        Me.grdDetall.Columns.Add(BaseUI.Common.GridView.NewColumn(tbl.締切日付Column, , , , , , 90))
        Me.grdDetall.Columns.Add(BaseUI.Common.GridView.NewColumn(tbl.締切残高Column, , , , Windows.Forms.DataGridViewContentAlignment.MiddleRight, "#,##0", 100))
    End Sub
    ...
    Private Sub grdDetall_RowBind(ByVal RowIndex As Integer) Handles grdDetall.RowBind
        ...
        Me.grdDetall.Rows(RowIndex).Cells(tbl.締切日付Column.ColumnName).Value = Frame.Fields.Detail1(RowIndex).締切日付.Value
        Me.grdDetall.Rows(RowIndex).Cells(tbl.締切残高Column.ColumnName).Value = Frame.Fields.Detail1(RowIndex).締切残高.Value
    End Sub
    ...
    Private Sub grdDetall_SelectionChanged(sender As Object, e As System.EventArgs) Handles grdDetall.SelectionChanged
        ...
        Me.txt締切日付.Text = Me.grdDetall.Rows(i).Cells(tbl.締切日付Column.ColumnName).Value
        Me.txt締切残高.Text = Me.grdDetall.Rows(i).Cells(tbl.締切残高Column.ColumnName).Value
    ...
    Private Sub grdDetall_RowUpdateExecute(ByVal RowIndex As Integer) Handles grdDetall.RowUpdateExecute
        Frame.Fields.Detail1(RowIndex).締切日付.Value = Me.txt締切日付.Text
        Frame.Fields.Detail1(RowIndex).締切残高.Value = Me.txt締切残高.Text
    End Sub
