Public Class frmBillDDInfo
    Inherits BaseUI.BaseForm

#Region "ﾌﾚｰﾑﾜｰｸ"
    Private Frame As NodeCore.Frame.Master.BillDDInfo
#End Region
...
#Region "定型準備"
    ...
    Protected Overrides Sub ControlInit()
        ...
        ' ﾃｷｽﾄﾎﾞｯｸｽ
        Me.txt締日.PropetyInitForm(BaseContents.Field.LockType.Key, False, Windows.Forms.ImeMode.Disable, BaseContents.TextBox.TextStyle.Code, NodeUI.Help.Type.M締日情報, Me.txt締日.Name, "")
        Me.txt請求日付.PropetyInitForm(BaseContents.Field.LockType.Data, False, Windows.Forms.ImeMode.Disable, BaseContents.TextBox.TextStyle.Code, NodeUI.Help.Type.M仕入先, "", "")
    ...
    Protected Overrides Sub FrameInit()
        ...
        ' ｺﾝｽﾄﾗｸﾀ
        '-------------------------------------
        Frame = New NodeCore.Frame.Master.BillDDInfo(Me.Security, Me.Config)

        '-------------------------------------
        ' 紐付け
        '-------------------------------------
        Frame.Fields.Header.締日.Control = Me.txt締日
        Frame.Fields.Header.請求日付.Control = Me.txt請求日付
...
#Region "ﾕｰｻﾞｲﾍﾞﾝﾄ"
    ...
    '''========================================================================================
    Private Sub txt締日_AfterUpdate(sender As Object, e As System.EventArgs) Handles txt締日.AfterUpdate
        '----------------------------------------------------------------------
        ' 該当ﾃﾞｰﾀ 読込
        '----------------------------------------------------------------------
        If Frame.Read() Then
