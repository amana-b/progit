Public Class frmMount
    Inherits BaseUI.BaseForm

#Region "ﾌﾚｰﾑﾜｰｸ"
    Private Frame As NodeCore.Frame.Master.Mount
#End Region
...
#Region "定型準備"
    ...
    Protected Overrides Sub ControlInit()
        ...
        ' ﾃｷｽﾄﾎﾞｯｸｽ
        Me.txt台紙CD.PropetyInitForm(BaseContents.Field.LockType.Key, False, Windows.Forms.ImeMode.Disable, BaseContents.TextBox.TextStyle.Code, NodeUI.Help.Type.M台紙, Me.txt台紙CD.Name, "")
        Me.txt台紙名.PropetyInitForm(BaseContents.Field.LockType.Data, False, Windows.Forms.ImeMode.Disable, BaseContents.TextBox.TextStyle.None, NodeUI.Help.Type.None, "", "")

        ' ﾈｰﾑﾎﾞｯｸｽ

        ' ﾄﾞﾛｯﾌﾟﾀﾞｳﾝ
        Me.ddl再利用区分.PropetyInitForm(BaseContents.Field.LockType.Data, False, NodeUI.List.Type.M区分_再利用区分)
    ...
    Protected Overrides Sub FrameInit()
        ...
        ' ｺﾝｽﾄﾗｸﾀ
        '-------------------------------------
        Frame = New NodeCore.Frame.Master.Mount(Me.Security, Me.Config)

        '-------------------------------------
        ' 紐付け
        '-------------------------------------
        Frame.Fields.Header.台紙CD.Control = Me.txt台紙CD
        Frame.Fields.Header.台紙名.Control = Me.txt台紙名
        Frame.Fields.Header.再利用区分.Control = Me.ddl再利用区分
...
#Region "ﾕｰｻﾞｲﾍﾞﾝﾄ"
    ...
    '''========================================================================================
    Private Sub txt糊CD_AfterUpdate(sender As Object, e As System.EventArgs) Handles txt台紙CD.AfterUpdate
        '----------------------------------------------------------------------
        ' 該当ﾃﾞｰﾀ 読込
        '----------------------------------------------------------------------
        If Frame.Read() Then
