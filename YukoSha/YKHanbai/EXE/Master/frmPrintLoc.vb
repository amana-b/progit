Public Class frmPrintLoc
    Inherits BaseUI.BaseForm

#Region "ﾌﾚｰﾑﾜｰｸ"
    Private Frame As NodeCore.Frame.Master.PrintLoc
#End Region
...
#Region "定型準備"
    ...
    Protected Overrides Sub ControlInit()
        ...
        ' ﾃｷｽﾄﾎﾞｯｸｽ
        Me.txt印刷場所CD.PropetyInitForm(BaseContents.Field.LockType.Key, False, Windows.Forms.ImeMode.Disable, BaseContents.TextBox.TextStyle.Code, NodeUI.Help.Type.M印刷場所, Me.txt印刷場所CD.Name, Me.txt印刷場所名.Name)
        Me.txt印刷場所名.PropetyInitForm(BaseContents.Field.LockType.Data, True, Windows.Forms.ImeMode.Disable, BaseContents.TextBox.TextStyle.None, NodeUI.Help.Type.None, "", "")
        Me.txt印刷機械種別CD.PropetyInitForm(BaseContents.Field.LockType.Data, False, Windows.Forms.ImeMode.Disable, BaseContents.TextBox.TextStyle.Code, NodeUI.Help.Type.M印刷機械種別, Me.txt印刷機械種別CD.Name, Me.nam印刷機械種別名.Name)

        ' ﾈｰﾑﾎﾞｯｸｽ
        Me.nam印刷機械種別名.PropetyInitForm(BaseContents.TextBox.TextStyle.None)

        ' ﾄﾞﾛｯﾌﾟﾀﾞｳﾝ
        Me.ddl場所区分.PropetyInitForm(BaseContents.Field.LockType.Data, True, NodeUI.List.Type.M区分_場所区分)
    ...
    Protected Overrides Sub FrameInit()
        ...
        ' ｺﾝｽﾄﾗｸﾀ
        '-------------------------------------
        Frame = New NodeCore.Frame.Master.PrintLoc(Me.Security, Me.Config)

        '-------------------------------------
        ' 紐付け
        '-------------------------------------
        Frame.Fields.Header.印刷場所CD.Control = Me.txt印刷場所CD
        Frame.Fields.Header.印刷場所名.Control = Me.txt印刷場所名
        Frame.Fields.Header.印刷機械種別CD.Control = Me.txt印刷機械種別CD
        Frame.Fields.Header.印刷機械種別名.Control = Me.nam印刷機械種別名
        Frame.Fields.Header.場所区分.Control = Me.ddl場所区分
...
#Region "ﾕｰｻﾞｲﾍﾞﾝﾄ"
    ...
    '''========================================================================================
    Private Sub txt印刷場所CD_AfterUpdate(sender As Object, e As System.EventArgs) Handles txt印刷場所CD.AfterUpdate
        '----------------------------------------------------------------------
        ' 該当ﾃﾞｰﾀ 読込
        '----------------------------------------------------------------------
        If Frame.Read() Then
