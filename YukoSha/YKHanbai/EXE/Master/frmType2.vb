...
#Region "ﾌﾚｰﾑﾜｰｸ"
    Private Frame As NodeCore.Frame.Master.Type2
#End Region
...
#Region "定型準備"
    ...
    Protected Overrides Sub ControlInit()
        ...
        ' ﾃｷｽﾄﾎﾞｯｸｽ
        Me.txt小分類CD.PropetyInitForm(BaseContents.Field.LockType.Key, True, Windows.Forms.ImeMode.Disable, BaseContents.TextBox.TextStyle.Code, NodeUI.Help.Type.M小分類, Me.txt小分類CD.Name, "")
        Me.txt小分類名.PropetyInitForm(BaseContents.Field.LockType.Data, True, Windows.Forms.ImeMode.On, BaseContents.TextBox.TextStyle.None, NodeUI.Help.Type.None, "", "")
    ...
    Protected Overrides Sub FrameInit()
        ...
        ' ｺﾝｽﾄﾗｸﾀ
        '-------------------------------------
        Frame = New NodeCore.Frame.Master.Type2(Me.Security, Me.Config)

        '-------------------------------------
        ' 紐付け
        '-------------------------------------
        Frame.Fields.Header.小分類CD.Control = Me.txt小分類CD
        Frame.Fields.Header.小分類名.Control = Me.txt小分類名
...
#Region "ﾕｰｻﾞｲﾍﾞﾝﾄ"
    ...
    '''========================================================================================
    Private Sub txt小分類CD_AfterUpdate(sender As Object, e As System.EventArgs) Handles txt小分類CD.AfterUpdate
        '----------------------------------------------------------------------
        ' 該当ﾃﾞｰﾀ 読込
        '----------------------------------------------------------------------
        If Frame.Read() Then
