...
#Region "定型準備"
    ...
    Protected Overrides Sub ControlInit()
        ...
        '-------------------------------------
        ' 品名CD
        Me.txtﾃﾞｰﾀ区分.HelpType = NodeUI.Help.Type.Mヤマトデータ区分
        Me.txtﾃﾞｰﾀ区分.HelpKeys = Me.txtﾃﾞｰﾀ区分.Name
        Me.txtﾃﾞｰﾀ区分.HelpLink = Me.txtﾃﾞｰﾀ区分名.Name

        ' 品名CD
        Me.txt品名CD.HelpType = NodeUI.Help.Type.M品名
        Me.txt品名CD.HelpKeys = Me.txt品名CD.Name
        Me.txt品名CD.HelpLink = Me.lbl品名.Name

        ' 光村印刷出荷先CD
        Me.txt光村印刷出荷先CD.HelpType = NodeUI.Help.Type.M出荷先
        Me.txt光村印刷出荷先CD.HelpKeys = Me.txt光村印刷得意先CD.Name & vbTab & Me.txt光村印刷出荷先CD.Name
        Me.txt光村印刷出荷先CD.HelpLink = Me.lbl光村印刷出荷先名.Name

        ' 光村印刷得意先CD
        Me.txt光村印刷得意先CD.HelpType = NodeUI.Help.Type.M得意先
        Me.txt光村印刷得意先CD.HelpKeys = Me.txt光村印刷得意先CD.Name
        Me.txt光村印刷得意先CD.HelpLink = Me.lbl光村印刷得意先名.Name

        '-------------------------------------
        ' ﾘｽﾄﾀｲﾌﾟ
        '-------------------------------------
        Me.ddl種別内訳区分.ListType = NodeUI.List.Type.M区分_種別内訳区分
    End Sub
    ...
    Protected Overrides Sub FrameInit()
        ...
        ' 紐付け
        '-------------------------------------
        Frame.Fields.Header.データ区分.Control = Me.txtﾃﾞｰﾀ区分
        Frame.Fields.Header.データ区分名.Control = Me.txtﾃﾞｰﾀ区分名
...
#Region "ﾕｰｻﾞｲﾍﾞﾝﾄ"
    ...
    '''========================================================================================
    Private Sub txtﾃﾞｰﾀ区分_AfterUpdate(sender As Object, e As System.EventArgs) Handles txtﾃﾞｰﾀ区分.AfterUpdate
        '----------------------------------------------------------------------
        ' 該当ﾃﾞｰﾀ 読込
        '----------------------------------------------------------------------
        If Frame.Read() Then
    ...
    Private Sub txt品名CD_AfterUpdate(sender As Object, e As System.EventArgs) Handles txt品名CD.AfterUpdate
        ...
        ' 変数定義
        '----------------------------------------------------------------------
        Dim ada As New NodeDatabase.DataSetViewTableAdapters.V0ヤマトデータ区分TableAdapter
        Dim tbl As New NodeDatabase.DataSetView.V0ヤマトデータ区分DataTable
        ...
        '----------------------------------------------------------------------
        qry.Clear()
        qry.Add(New BaseDatabase.Condition(tbl.データ区分Column.ColumnName, Me.txtﾃﾞｰﾀ区分.Text, BaseDatabase.Contents.Compare.Equal))
...
#Region "ﾌｧﾝｸｼｮﾝ"
    ...
    Protected Overrides Sub btnF06_Click(sender As Object, e As System.EventArgs)
        ...
        Frame.Fields.Header.略称.Value = Replace(Frame.Fields.Header.データ区分名.Value.ToString.PadRight(4, "　").ToString.Substring(0, 4), "　", "")
    End Sub
