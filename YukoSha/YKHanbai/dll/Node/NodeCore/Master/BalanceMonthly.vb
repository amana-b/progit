Namespace Frame.Master
    Partial Public Class BalanceMonthly
...
#Region "ﾌｨｰﾙﾄﾞ定義"
        Public Class Header
            ...
            ''' ----------------------------------------------------------------------------------------
            Public 取引先区分 As BaseCore.Common.Field.ItemData
            Public 取引先CD As BaseCore.Common.Field.ItemData
            Public 取引先名 As BaseCore.Common.Field.ItemData
            Public 締日 As BaseCore.Common.Field.ItemData

            Public 月次年月 As BaseCore.Common.Field.ItemData
            Public 月次残高 As BaseCore.Common.Field.ItemData
            ...
            Public Sub New(ByVal Config As NodeCore.Common.Config)
                ...
                ' ｲﾝｽﾀﾝｽ(基本)
                '----------------------------------------------------------------------
                Me.取引先区分 = New BaseCore.Common.Field.ItemData(tbl.取引先区分Column)
                Me.取引先CD = New BaseCore.Common.Field.ItemData(tbl.取引先CDColumn)
                Me.取引先名 = New BaseCore.Common.Field.ItemData(tbl.取引先名Column)
                Me.締日 = New BaseCore.Common.Field.ItemData(tbl.締日Column)

                Me.月次年月 = New BaseCore.Common.Field.ItemData(tbl.月次年月Column)
                Me.月次残高 = New BaseCore.Common.Field.ItemData(tbl.月次残高Column, NodeContents.Constant.桁数.取引額.整数, NodeContents.Constant.桁数.取引額.小数)
            End Sub
        End Class
...
#Region "Clear"
        ...
        Protected Overrides Function Clear_Header(Optional ByVal KeyClear As Boolean = True) As Boolean
            ...
            '----------------------------------------------------------------------
            If KeyClear Then
                Me.Fields.Header.取引先区分.Value = NodeContents.Constant.CodeValue.取引先区分.得意先
                Me.Fields.Header.取引先CD.Value = Nothing
                Me.Fields.Header.取引先名.Value = Nothing
            ...
            ' ﾃﾞｰﾀ部ｸﾘｱ
            '----------------------------------------------------------------------
            Me.Fields.Header.月次年月.Value = Nothing
            Me.Fields.Header.月次残高.Value = Nothing
...
#Region "Read"
        ...
        Protected Overrides Function Read_Header() As Boolean
            ...
            ' 変数定義
            '----------------------------------------------------------------------
            Dim ada As New NodeDatabase.DataSetViewTableAdapters.V0残高月次TableAdapter
            Dim tbl As New NodeDatabase.DataSetView.V0残高月次DataTable
            ...
            '----------------------------------------------------------------------
            qry.Clear()
            qry.Add(New BaseDatabase.Condition(tbl.取引先区分Column.ColumnName, Me.Fields.Header.取引先区分.Value, BaseDatabase.Contents.Compare.Equal))
            qry.Add(New BaseDatabase.Condition(tbl.取引先CDColumn.ColumnName, Me.Fields.Header.取引先CD.Value, BaseDatabase.Contents.Compare.Equal))
            ...
            If tbl.Count > 0 Then
                Me.Fields.Header.取引先区分.Value = tbl(0).Item(tbl.取引先区分Column.ColumnName)
                Me.Fields.Header.取引先CD.Value = tbl(0).Item(tbl.取引先CDColumn.ColumnName)
                Me.Fields.Header.取引先名.Value = tbl(0).Item(tbl.取引先名Column.ColumnName)
...
#Region "Valid(Write)"
        ...
        Protected Overrides Function Valid_Write_Header_Common() As NodeContents.ActionButton.ErrorLevel
            ...
            ' ｴﾗｰｸﾘｱ
            '------------------------------------------------------------------
            Me.Fields.Header.取引先区分.IsError = False
            Me.Fields.Header.取引先CD.IsError = False
            Me.Fields.Header.取引先名.IsError = False
            Me.Fields.Header.締日.IsError = False

            '------------------------------------------------------------------
            ' 共通検査
            '------------------------------------------------------------------
            Validator.BaseChecker(Me.Fields.Header.取引先区分, strMsg, Me.ControlError)
            Validator.BaseChecker(Me.Fields.Header.取引先CD, strMsg, Me.ControlError)
            Validator.BaseChecker(Me.Fields.Header.取引先名, strMsg, Me.ControlError)
        ...
        Protected Overrides Function Valid_Write_Header_Another() As NodeContents.ActionButton.ErrorLevel
            ...
            '-------------------------------------
            If strMsg = "" Then
                Select Case Me.Fields.Header.取引先区分.Value
                    Case NodeContents.Constant.CodeValue.取引先区分.得意先
                        ...
                        qry.Clear()
                        qry.Add(New BaseDatabase.Condition(tbl.得意先CDColumn.ColumnName, Me.Fields.Header.取引先CD.Value, BaseDatabase.Contents.Compare.Equal))
                        tbl = ada.SelectByCommon(qry)
                        If tbl.Count = 0 Then
                            Validator.SetMessage(strMsg, "得意先マスタに未登録です。", Me.ControlError, Me.Fields.Header.取引先CD)
                        End If
                    ...
                    Case NodeContents.Constant.CodeValue.取引先区分.仕入先
                        ...
                        qry.Clear()
                        qry.Add(New BaseDatabase.Condition(tbl.仕入先CDColumn.ColumnName, Me.Fields.Header.取引先CD.Value, BaseDatabase.Contents.Compare.Equal))
                        tbl = ada.SelectByCommon(qry)
                        If tbl.Count = 0 Then
                            Validator.SetMessage(strMsg, "仕入先マスタに未登録です。", Me.ControlError, Me.Fields.Header.取引先CD)
                        End If
                End Select
            End If
...
#Region "Valid(RowAdd)"
        ...
        Protected Overrides Function Valid_RowAdd_Header_Common() As NodeContents.ActionButton.ErrorLevel
            ...
            ' ｴﾗｰｸﾘｱ
            '------------------------------------------------------------------
            Me.Fields.Header.月次年月.IsError = False
            Me.Fields.Header.月次残高.IsError = False

            '------------------------------------------------------------------
            ' 共通検査
            '------------------------------------------------------------------
            Validator.BaseChecker(Me.Fields.Header.月次年月, strMsg, Me.ControlError)
            Validator.BaseChecker(Me.Fields.Header.月次残高, strMsg, Me.ControlError)
...
#Region "Clear"
        ...
        Public Overrides Function Clear_Detail1(ByVal Index As Integer) As Boolean
            Me.Fields.Detail1(Index).月次年月.Value = Nothing
            Me.Fields.Detail1(Index).月次残高.Value = Nothing
        End Function
#End Region
...
#Region "Read"
        ...
        Protected Overrides Function Read_Detail1() As Boolean
            ...
            strSort &= strDelim & tbl.月次年月Column.ColumnName : strDelim = ","
            ...
            For i As Integer = 0 To tbl.Rows.Count - 1
                Me.Fields.Detail1(i).月次年月.Value = tbl(i).Item(tbl.月次年月Column.ColumnName)
                Me.Fields.Detail1(i).月次残高.Value = tbl(i).Item(tbl.月次残高Column.ColumnName)
            Next
...
#Region "Write"
        ...
        Protected Overrides Function Write_Detail1() As Boolean
            ...
            ' 変数定義
            '----------------------------------------------------------------------
            Dim ada As New NodeDatabase.DataSetTranTableAdapters.T残高月次TableAdapter
            Dim qry As New Collection
            ...
            Try
                Using scope As New System.Transactions.TransactionScope(System.Transactions.TransactionScopeOption.Required, TimeSpan.Zero)
                    ...
                    For i As Integer = 0 To Me.Fields.Detail1.Count - 1
                        ...
                        ada.Insert(Me.Fields.Header.取引先区分.Value _
                                 , Me.Fields.Header.取引先CD.Value _
                                 , Me.Fields.Detail1(i).月次年月.Value _
                                 , BaseCore.Common.Text.CVal(Me.Fields.Detail1(i).月次残高.Value) _
                                  )
                    Next
                    '-------------------------------------
                    ' COMMIT
...
#Region "Delete"
        ...
        Protected Overrides Function Delete_Detail1() As Boolean
            ...
            '----------------------------------------------------------------------
            Dim ada As New NodeDatabase.DataSetTranTableAdapters.T残高月次TableAdapter
            Dim tbl As New NodeDatabase.DataSetTran.T残高月次DataTable
            Dim qry As New Collection
...
#Region "Valid(Write)"
        ...
        Protected Overrides Function Valid_Write_Detail1_Common() As NodeContents.ActionButton.ErrorLevel
            ...
            '------------------------------------------------------------------
            For i As Integer = 0 To Me.Fields.Detail1.Count - 1
                Me.Fields.Detail1(i).月次年月.IsError = False
                Me.Fields.Detail1(i).月次残高.IsError = False
            Next
            ...
            For i As Integer = 0 To Me.Fields.Detail1.Count - 1
                ...
                Validator.BaseChecker(Me.Fields.Detail1(i).月次年月, strMsg, Me.ControlError)
                Validator.BaseChecker(Me.Fields.Detail1(i).月次残高, strMsg, Me.ControlError)
            Next
        ...
        Protected Overrides Function Valid_Write_Detail1_Another() As NodeContents.ActionButton.ErrorLevel
            ...
            If strMsg = "" Then
                ...
                For i As Integer = 0 To Me.Fields.Detail1.Count - 1
                    ...
                    strKey = Me.Fields.Detail1(i).月次年月.Value
                    If arrDupl.ContainsKey(strKey) Then
                        Validator.SetMessage(strMsg, "月次年月が重複している為、処理できません。(@0)".Replace("@0", strKey), Me.ControlError, Me.Fields.Header.月次年月)
                        Exit For
                    End If
...
#Region "無効行判定"
        ...
        Protected Overrides Function Empty_Detail1(ByVal Detail As Object) As Boolean
            ...
            '------------------------------------------------------------------
            Select Case True
                Case Not BaseCore.Common.Text.IsEmptyOrZero(row.月次年月.Value) : Return False
            End Select
...
#Region "行複写"
        ...
        Protected Function CopyValue_Detail1(ByVal IndexF As Integer, ByVal IndexT As Integer) As Boolean
            Me.Fields.Detail1(IndexT).月次年月.Value = Me.Fields.Detail1(IndexF).月次年月.Value
            Me.Fields.Detail1(IndexT).月次残高.Value = Me.Fields.Detail1(IndexF).月次残高.Value
        End Function
#End Region
