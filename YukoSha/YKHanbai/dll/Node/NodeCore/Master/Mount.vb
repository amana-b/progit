...
#Region "ﾌｨｰﾙﾄﾞ定義"
        Public Class Header
            ...
            Public 再利用区分 As BaseCore.Common.Field.ItemData
            ...
            Public Sub New(ByVal Config As NodeCore.Common.Config)
                ...
                Me.再利用区分 = New BaseCore.Common.Field.ItemData(tbl.再利用区分Column)
...
#Region "Read"
        ...
        Protected Overrides Function Read_Header() As Boolean
            ...
            '----------------------------------------------------------------------
            qry.Clear()
            qry.Add(New BaseDatabase.Condition(tbl.台紙CDColumn.ColumnName, Me.Fields.Header.台紙CD.Value, BaseDatabase.Contents.Compare.Equal))
            ...
            Me.Fields.Header.台紙CD.Value = tbl(0).Item(tbl.台紙CDColumn.ColumnName)
            Me.Fields.Header.台紙名.Value = tbl(0).Item(tbl.台紙名Column.ColumnName)
            Me.Fields.Header.再利用区分.Value = tbl(0).Item(tbl.再利用区分Column.ColumnName)
        ...
        Protected Overrides Function Write_Header() As Boolean
            ...
            Using scope As New System.Transactions.TransactionScope(System.Transactions.TransactionScopeOption.Required, TimeSpan.Zero)
                If tbl.Count = 0 Then
                    ...
                    ' 新規
                    '-------------------------------------
                    ada.Insert(Me.Fields.Header.台紙CD.Value _
                             , Me.Fields.Header.台紙名.Value _
                             , Me.Fields.Header.再利用区分.Value _
                              )
                Else
                    '-------------------------------------
                    ' 訂正
                    '-------------------------------------
                    tbl(0).Item(tbl.台紙CDColumn.ColumnName) = Me.Fields.Header.台紙CD.Value
                    tbl(0).Item(tbl.台紙名Column.ColumnName) = Me.Fields.Header.台紙名.Value
                    tbl(0).Item(tbl.再利用区分Column.ColumnName) = Me.Fields.Header.再利用区分.Value
                    ada.Update(tbl)
                End If
...
#Region "Valid_Write"
        ...
        Protected Overrides Function Valid_Write_Header_Common() As NodeContents.ActionButton.ErrorLevel
            ...
            Me.Fields.Header.再利用区分.IsError = False
