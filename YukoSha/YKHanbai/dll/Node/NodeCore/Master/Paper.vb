...
#Region "ﾌｨｰﾙﾄﾞ定義"
        Public Class Header
            ...
            Public 紙種別区分 As BaseCore.Common.Field.ItemData
            ...
            Public Sub New(ByVal Config As NodeCore.Common.Config)
                ...
                Me.紙種別区分 = New BaseCore.Common.Field.ItemData(tbl.紙種別区分Column)
...
#Region "Clear"
        ...
        Protected Overrides Function Clear_Header(Optional ByVal KeyClear As Boolean = True) As Boolean
            ...
            '----------------------------------------------------------------------
            If KeyClear Then
                Me.Fields.Header.紙CD.Value = Nothing
            ...
            ' ﾃﾞｰﾀ部ｸﾘｱ
            '----------------------------------------------------------------------
            Me.Fields.Header.紙名.Value = Nothing
            Me.Fields.Header.紙種別区分.Value = Nothing
...
#Region "Read"
        ...
        Protected Overrides Function Read_Header() As Boolean
            ...
            '----------------------------------------------------------------------
            qry.Clear()
            qry.Add(New BaseDatabase.Condition(tbl.紙CDColumn.ColumnName, Me.Fields.Header.紙CD.Value, BaseDatabase.Contents.Compare.Equal))
            ...
            Me.Fields.Header.紙CD.Value = tbl(0).Item(tbl.紙CDColumn.ColumnName)
            Me.Fields.Header.紙名.Value = tbl(0).Item(tbl.紙名Column.ColumnName)
            Me.Fields.Header.紙種別区分.Value = tbl(0).Item(tbl.紙種別区分Column.ColumnName)
        ...
        Protected Overrides Function Write_Header() As Boolean
            ...
            Using scope As New System.Transactions.TransactionScope(System.Transactions.TransactionScopeOption.Required, TimeSpan.Zero)
                If tbl.Count = 0 Then
                    ...
                    ' 新規
                    '-------------------------------------
                    ada.Insert(Me.Fields.Header.紙CD.Value _
                             , Me.Fields.Header.紙名.Value _
                             , Me.Fields.Header.紙種別区分.Value _
                              )
                Else
                    '-------------------------------------
                    ' 訂正
                    '-------------------------------------
                    tbl(0).Item(tbl.紙CDColumn.ColumnName) = Me.Fields.Header.紙CD.Value
                    tbl(0).Item(tbl.紙名Column.ColumnName) = Me.Fields.Header.紙名.Value
                    tbl(0).Item(tbl.紙種別区分Column.ColumnName) = Me.Fields.Header.紙種別区分.Value
                    ada.Update(tbl)
                End If
...
#Region "Valid_Write"
        ...
        Protected Overrides Function Valid_Write_Header_Common() As NodeContents.ActionButton.ErrorLevel
            ...
            ' ｴﾗｰｸﾘｱ
            '------------------------------------------------------------------
            Me.Fields.Header.紙CD.IsError = False
            Me.Fields.Header.紙名.IsError = False
            Me.Fields.Header.紙種別区分.IsError = False

            '------------------------------------------------------------------
            ' 共通検査
            '------------------------------------------------------------------
            Validator.BaseChecker(Me.Fields.Header.紙CD, strMsg, Me.ControlError)
            Validator.BaseChecker(Me.Fields.Header.紙名, strMsg, Me.ControlError)
            Validator.BaseChecker(Me.Fields.Header.紙種別区分, strMsg, Me.ControlError)
