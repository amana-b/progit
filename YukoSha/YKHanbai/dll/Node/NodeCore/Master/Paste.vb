...
#Region "Clear"
        ...
        Protected Overrides Function Clear_Header(Optional ByVal KeyClear As Boolean = True) As Boolean
            ...
            '----------------------------------------------------------------------
            If KeyClear Then
                Me.Fields.Header.糊CD.Value = Nothing
            ...
            ' ﾃﾞｰﾀ部ｸﾘｱ
            '----------------------------------------------------------------------
            Me.Fields.Header.糊名.Value = Nothing
...
#Region "Read"
        ...
        Protected Overrides Function Read_Header() As Boolean
            ...
            '----------------------------------------------------------------------
            qry.Clear()
            qry.Add(New BaseDatabase.Condition(tbl.糊CDColumn.ColumnName, Me.Fields.Header.糊CD.Value, BaseDatabase.Contents.Compare.Equal))
            ...
            Me.Fields.Header.糊CD.Value = tbl(0).Item(tbl.糊CDColumn.ColumnName)
            Me.Fields.Header.糊名.Value = tbl(0).Item(tbl.糊名Column.ColumnName)
        ...
        Protected Overrides Function Write_Header() As Boolean
            ...
            Using scope As New System.Transactions.TransactionScope(System.Transactions.TransactionScopeOption.Required, TimeSpan.Zero)
                If tbl.Count = 0 Then
                    ...
                    ' 新規
                    '-------------------------------------
                    ada.Insert(Me.Fields.Header.糊CD.Value _
                             , Me.Fields.Header.糊名.Value _
                              )
                Else
                    '-------------------------------------
                    ' 訂正
                    '-------------------------------------
                    tbl(0).Item(tbl.糊CDColumn.ColumnName) = Me.Fields.Header.糊CD.Value
                    tbl(0).Item(tbl.糊名Column.ColumnName) = Me.Fields.Header.糊名.Value
                    ada.Update(tbl)
                End If
...
#Region "Valid_Write"
        ...
        Protected Overrides Function Valid_Write_Header_Common() As NodeContents.ActionButton.ErrorLevel
            ...
            ' ｴﾗｰｸﾘｱ
            '------------------------------------------------------------------
            Me.Fields.Header.糊CD.IsError = False
            Me.Fields.Header.糊名.IsError = False

            '------------------------------------------------------------------
            ' 共通検査
            '------------------------------------------------------------------
            Validator.BaseChecker(Me.Fields.Header.糊CD, strMsg, Me.ControlError)
            Validator.BaseChecker(Me.Fields.Header.糊名, strMsg, Me.ControlError)
