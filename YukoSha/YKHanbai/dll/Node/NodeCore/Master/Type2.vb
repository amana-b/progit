...
#Region "ﾌｨｰﾙﾄﾞ定義"
        Public Class Header
            ...
            ''' ----------------------------------------------------------------------------------------
            Public 小分類CD As BaseCore.Common.Field.ItemData
            Public 小分類名 As BaseCore.Common.Field.ItemData
            ...
            Public Sub New(ByVal Config As NodeCore.Common.Config)
                ...
                ' ｲﾝｽﾀﾝｽ(基本)
                '----------------------------------------------------------------------
                Me.小分類CD = New BaseCore.Common.Field.ItemData(tbl.小分類CDColumn)
                Me.小分類名 = New BaseCore.Common.Field.ItemData(tbl.小分類名Column)
...
#Region "Clear"
        ...
        Protected Overrides Function Clear_Header(Optional ByVal KeyClear As Boolean = True) As Boolean
            ...
            '----------------------------------------------------------------------
            If KeyClear Then
                Me.Fields.Header.小分類CD.Value = Nothing
            ...
            ' ﾃﾞｰﾀ部ｸﾘｱ
            '----------------------------------------------------------------------
            Me.Fields.Header.小分類名.Value = Nothing
...
#Region "Read"
        ...
        Protected Overrides Function Read_Header() As Boolean
            ...
            '----------------------------------------------------------------------
            qry.Clear()
            qry.Add(New BaseDatabase.Condition(tbl.小分類CDColumn.ColumnName, Me.Fields.Header.小分類CD.Value, BaseDatabase.Contents.Compare.Equal))
            ...
            Me.Fields.Header.小分類CD.Value = tbl(0).Item(tbl.小分類CDColumn.ColumnName)
            Me.Fields.Header.小分類名.Value = tbl(0).Item(tbl.小分類名Column.ColumnName)
...
#Region "Write"
        ...
        Protected Overrides Function Write_Header() As Boolean
            ...
            Dim ada As New NodeDatabase.DataSetMasterTableAdapters.M小分類TableAdapter
            Dim tbl As New NodeDatabase.DataSetMaster.M小分類DataTable
            Dim qry As New Collection
            ...
            Using scope As New System.Transactions.TransactionScope(System.Transactions.TransactionScopeOption.Required, TimeSpan.Zero)
                If tbl.Count = 0 Then
                    ...
                    ' 新規
                    '-------------------------------------
                    ada.Insert(Me.Fields.Header.小分類CD.Value _
                             , Me.Fields.Header.小分類名.Value _
                                                  )
                Else
                    '-------------------------------------
                    ' 訂正
                    '-------------------------------------
                    tbl(0).Item(tbl.小分類CDColumn.ColumnName) = Me.Fields.Header.小分類CD.Value
                    tbl(0).Item(tbl.小分類名Column.ColumnName) = Me.Fields.Header.小分類名.Value
                    ada.Update(tbl)
                End If
...
#Region "Valid_Write"
        ...
        Protected Overrides Function Valid_Write_Header_Common() As NodeContents.ActionButton.ErrorLevel
            ...
            ' ｴﾗｰｸﾘｱ
            '------------------------------------------------------------------
            Me.Fields.Header.小分類CD.IsError = False
            Me.Fields.Header.小分類名.IsError = False

            '------------------------------------------------------------------
            ' 共通検査
            '------------------------------------------------------------------
            Validator.BaseChecker(Me.Fields.Header.小分類CD, strMsg, Me.ControlError)
            Validator.BaseChecker(Me.Fields.Header.小分類名, strMsg, Me.ControlError)
