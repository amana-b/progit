...
Public Class OrderRegister
    ...
    '''========================================================================================
    Private Sub OrderRegister_ReportStart(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.ReportStart
        '------------------------------------------------------------------------------------------
        ' ﾍﾟｰｼﾞ 設定
        '------------------------------------------------------------------------------------------
        Dim arrControls As New ArrayList
        Dim tbl As New NodeDatabase.DataSetTran.T受注基本DataTable

        '------------------------------------------------------------------------------------------
        ' ﾀｲﾄﾙ 設定
        '------------------------------------------------------------------------------------------
        Me.txt受注台帳.Text = "受注台帳"
        ...
        ' ｸﾞﾙｰﾌﾟ ﾌｯﾀ
        '------------------------------------------------------------------------------------------
        Me.grpFtr受注.NewPage = NewPage.After
        ...
        'Header
        'Me.txt作業No.DataField = tbl.作業NOColumn.ColumnName
        'Me.txt得意先CD.DataField = tbl.得意先CDColumn.ColumnName
        'Me.txt得意先名.DataField = tbl.得意先名Column.ColumnName
        'Me.txt商品名.DataField = tbl.代表商品名Column.ColumnName
        'Me.txt機械別.DataField = tbl.機械別Column.ColumnName
        'Me.txt枚数.DataField = tbl.締日名Column.ColumnName
        'Me.txt通数.DataField = tbl.回収条件設定金額Column.ColumnName
        'Me.txtベース名.DataField = tbl.回収条件以上回収月名Column.ColumnName
        'Me.txt糊名.DataField = tbl.回収条件以上回収日名Column.ColumnName
        'Me.txt台紙名.DataField = tbl.回収条件以上回収方法名Column.ColumnName
        'Me.txt寸法.DataField = tbl.回収条件設定金額Column.ColumnName
        'Me.txt発注日.DataField = tbl.納品日付AColumn.ColumnName
        'Me.txt仕入先.DataField = tbl.回収条件未満回収日名Column.ColumnName
        'Me.txt入力担当者.DataField = tbl.入力担当者CDColumn.ColumnName
        'Me.txt受注単価.DataField = tbl.受注単価Column.ColumnName
        'Me.txt客先納期.DataField = tbl.納品日付BColumn.ColumnName
        ...
        ' 総合計
        Me.txt得意先受注日計.SummaryType = SummaryType.SubTotal
        Me.txt得意先受注日計.SummaryFunc = SummaryFunc.Sum
        Me.txt得意先受注日計.SummaryRunning = SummaryRunning.None
        Me.txt得意先受注日計.SummaryGroup = Me.Detail.Name
        'Me.txt得意先受注日計.DataField = tbl.合計数Column.ColumnName

        Me.txt商品受注金額.SummaryType = SummaryType.SubTotal
        Me.txt商品受注金額.SummaryFunc = SummaryFunc.Sum
        Me.txt商品受注金額.SummaryRunning = SummaryRunning.None
        Me.txt商品受注金額.SummaryGroup = Me.Detail.Name
        'Me.txt商品受注金額.DataField = tbl.合計数Column.ColumnName
        '------------------------------------------------------------------------------------------
        ' 書式設定
        '------------------------------------------------------------------------------------------
        ' Header
        ...
        ' Detail
        Me.txt枚数.OutputFormat = "#,###"
        Me.txt枚数計.OutputFormat = "#,###"
        Me.txt通数.OutputFormat = "#,###"
        Me.txt通数計.OutputFormat = "#,###"
        Me.txt受注単価.OutputFormat = "##.00"
        Me.txt商品受注金額.OutputFormat = "#,###"
    ...
    Private Sub grpFtr得意先CD_Format(sender As System.Object, e As System.EventArgs) Handles grpFtr受注.Format
